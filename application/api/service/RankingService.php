...
//            ->where('sign_time', '>', 0)
    ...
    public static function insertionSortByHours($arrays = [])
    {
        ...
        foreach ($arrays as $key => $array)
        {
            $newArrays[] =
                [
                    'portraitUrl' => config('api.api_url').config('api.source_path').$array['portraitUrl']?config('api.api_url').config('api.source_path').$array['portraitUrl']:config('api.api_url').config('api.source_path').'moRen.jpg',
                    'placing' => $key + 1,
